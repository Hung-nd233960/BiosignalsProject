...
def experiment_bin_distributions():
    ...
    fig.suptitle(
        "Figure B.5 — DFT bin distributions under white Gaussian noise",
        fontsize=11,
        fontstyle="italic",
    )
...
def experiment_spectral_detection():
    ...
    fig_td, _ = plot_time_domain(
        t,
        x,
        title="Tone buried in noise (time domain)",
        fig_id="Figure B.6",
        t_range=(0, 2),
    )
    ...
    for gamma, color in zip(GAMMA_VALUES, colors):
        ...
        ax_db.axhline(
            10 * np.log10(max(threshold, 1e-20)),  # same threshold in dB
            color=color,
            ls="--",
            lw=1,
            label=label,
...
def experiment_welch_averaging():
    ...
    fig, axes = plt.subplots(
        n_panels, 2, figsize=(14, 3 * n_panels), sharex="col"
    ...
    for i, (seg_dur, label) in enumerate(WELCH_SEGMENTS):
        ...
        f_welch, Pxx = sp_signal.welch(
            x,
            fs=FS,  # scipy Welch implementation
            nperseg=nperseg,
            noverlap=noverlap,
            window="hann",
        )
        ...
        axes[i, 0].set_title(f"{label} — L={n_segs}, Δf={delta_f:.3f} Hz", fontsize=9)
        ...
        axes[i, 1].set_title(f"{label} — L={n_segs}, Δf={delta_f:.3f} Hz", fontsize=9)
    ...
    fig.suptitle(
        "Figure B.8 — Welch averaging: variance vs. resolution tradeoff",
        fontsize=11,
        fontstyle="italic",
    )
    fig.set_dpi(DPI)  # project DPI
