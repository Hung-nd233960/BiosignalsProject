...
def figure_b9_dirichlet_anatomy():
    ...
    ax.plot(
        bins[plot_range], D_mag[plot_range], color="#2563eb", linewidth=1.5, zorder=3
    )
    ...
    ax.set_title(
        f"Figure B.9 — Dirichlet kernel anatomy ($M = {M}$)",
        fontsize=12,
        fontstyle="italic",
        loc="left",
    )
...
def figure_b10_dirichlet_full():
    ...
    ax.plot(
        ...
        linewidth=1.0,
        alpha=0.7,
        label="Envelope: $1/(M\\,|\\sin(\\omega/2)|)$",
    )
    ax.plot(-env_bins, envelope, "--", color="#dc2626", linewidth=1.0, alpha=0.7)

    ax.set_xlabel("Frequency (bins)", fontsize=11)
    ax.set_ylabel("Normalized magnitude", fontsize=11)
    ax.set_title(
        f"Figure B.10 — Dirichlet kernel with envelope ($M = {M}$)",
        fontsize=12,
        fontstyle="italic",
        loc="left",
    )
...
def figure_b11_first_sidelobe():
    ...
    print(
        f"  True maximum:     bin={true_bin:.3f}, magnitude={true_val:.5f}, {true_dB:.1f} dB"
    )
    print(
        f"  k=1.5 approx:     bin={approx_bin:.3f}, magnitude={approx_val:.5f}, {approx_dB:.1f} dB"
    )
    ...
    ax.annotate(
        f"True max: ({true_bin:.3f}, {true_val:.5f})\n= {true_dB:.1f} dB",
    ...
    ax.set_ylabel("Normalized magnitude", fontsize=11)
    ax.set_title(
        f"Figure B.11 — First side-lobe analysis ($M = {M}$)",
        fontsize=12,
        fontstyle="italic",
        loc="left",
    )
...
def figure_b12_decay_regression():
    ...
    for i in range(n_lobes):
        ...
        print(
            f"  Lobe {i+1}: actual bin={pk_b[i]:.3f}, mag={pk_v[i]:.5f} ({pk_dB[i]:.1f} dB)"
    ...
    print(
        f"    Slope: {slope:.3f}, R²: {r_value**2:.5f}, Decay: {dB_per_octave:.1f} dB/oct"
    )
    print("  Regression (crude midpoints):")
    print(
        f"    Slope: {slope_c:.3f}, R²: {r_c**2:.5f}, Decay: {dB_oct_crude:.1f} dB/oct"
    )
    ...
    ax2.set_title(
        "Log-log regression → decay rate", fontsize=10, fontstyle="italic", loc="left"
    )
    ...
    fig.suptitle(
        f"Figure B.12 — Side-lobe decay analysis ($M = {M}$)",
        fontsize=12,
        fontstyle="italic",
    )
    fig.set_dpi(DPI)
        ...
        table_data.append(
            {
                "lobe": i + 1,
                "bin_actual": pk_b[i],
                "bin_midpoint": crude_approx_bins[i],
                "mag_actual": pk_v[i],
                "mag_approx": env_approx,
                "dB_actual": pk_dB[i],
                "dB_approx": 20 * np.log10(env_approx),
            }
        )
...
def figure_b13_comparison():
    ...
    ax.set_ylabel("Normalized magnitude", fontsize=11)
    ax.set_title(
        f"Figure B.13 — Window spectra comparison, linear scale ($M = {M}$)",
        fontsize=12,
        fontstyle="italic",
        loc="left",
    )
...
def figure_b14_comparison_full():
    ...
    ax.set_ylabel("Normalized magnitude", fontsize=11)
    ax.set_title(
        f"Figure B.14 — Window spectra comparison, full range ($M = {M}$)",
        fontsize=12,
        fontstyle="italic",
        loc="left",
    )
