...
def _validate_colormap(cmap_name):
    ...
    assert cmap_name not in FORBIDDEN_COLORMAPS, (
        f"Colormap '{cmap_name}' is forbidden — use viridis, inferno, plasma, or magma"
    )


def save_figure(fig, lab_number, fig_id):
    """
    Save a figure to results/graphs/labN/ in all configured formats (PNG + SVG).

    Parameters
    ----------
    fig        : matplotlib Figure
    lab_number : int — lab number (1–7)
    ...
    for fmt in FIGURE_FORMATS:                             # save in each format
...
def plot_time_domain(t, x, xlabel="Time (s)", ylabel="Amplitude",
    ...
    if fig_id:
        ax.set_title(fig_id, loc="left", fontsize=9,      # figure number top-left
                     fontstyle="italic")
...
def plot_dual_stack_spectrum(freqs, power, xlabel="Frequency (Hz)",
    ...
    """
    fig, (ax_lin, ax_db) = plt.subplots(2, 1, figsize=(12, 6),  # two vertically stacked
                                         sharex=True)
    ...
    if fig_id:
        ax_lin.set_title(fig_id, loc="left", fontsize=9,   # figure number
                         fontstyle="italic")
...
def plot_spectrogram(t_stft, f_stft, Sxx, xlabel="Time (s)",
                     ylabel="Frequency (Hz)", title=None, fig_id=None,
                     cmap=COLORMAP, f_range=None, t_range=None):
    """
    ...
    fig, (ax_lin, ax_db) = plt.subplots(2, 1, figsize=(12, 8),
                                         sharex=True, sharey=True)

    # --- Top: linear scale ---
    im_lin = ax_lin.pcolormesh(t_stft, f_stft, Sxx,       # linear power
    ...
    if fig_id:
        ax_lin.set_title(fig_id, loc="left", fontsize=9,
                         fontstyle="italic")

    # --- Bottom: dB scale ---
    Sxx_db = 10 * np.log10(np.maximum(Sxx, 1e-20))        # dB conversion
    im_db = ax_db.pcolormesh(t_stft, f_stft, Sxx_db,      # dB power
                              shading="gouraud", cmap=cmap)
