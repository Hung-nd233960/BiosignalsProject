...
from src.common import (                                  # project utilities
    FS, DURATION, SEED, DPI, N_SAMPLES,
    make_tone, make_mixed_tones, make_time_axis,
...
def experiment_on_vs_off_bin():
    ...
    print(f"On-bin tone:  f0 = {F0_ON_BIN} Hz, f0/Δf = {F0_ON_BIN/delta_f:.1f} (integer = on-bin)")
    print(f"Off-bin tone: f0 = {F0_OFF_BIN} Hz, f0/Δf = {F0_OFF_BIN/delta_f:.1f} (half-integer = maximally off-bin)")
    ...
    fig1, _ = plot_time_domain(t_on, x_on, title="On-bin tone at 10.0 Hz (time domain)",
                                fig_id="Figure B.1a", t_range=t_show)
    fig2, _ = plot_time_domain(t_off, x_off, title="Off-bin tone at 10.5 Hz (time domain)",
                                fig_id="Figure B.1b", t_range=t_show)

    # --- Dual-stack spectrum plots ---
    f_show = (0, 30)                                      # zoom to 0–30 Hz
    fig3, _ = plot_dual_stack_spectrum(freqs_on, P_on,
                                       title="On-bin tone (10.0 Hz) — no leakage",
                                       fig_id="Figure B.2a", f_range=f_show)
    fig4, _ = plot_dual_stack_spectrum(freqs_off, P_off,
                                       title="Off-bin tone (10.5 Hz) — maximum leakage",
                                       fig_id="Figure B.2b", f_range=f_show)

    return {"delta_f": delta_f, "N": N,
...
def experiment_dual_tone():
    ...
    """
    # --- Generate signal ---
    x, n, t = make_mixed_tones([F1_CHORD, F2_CHORD], amplitudes=[A, A],
    ...
    # --- Time-domain (first) ---
    fig1, _ = plot_time_domain(t, x, title="Dual-tone chord (time domain)",
                                fig_id="Figure B.3a", t_range=(0, 0.5))

    # --- Dual-stack spectrum ---
    fig2, _ = plot_dual_stack_spectrum(freqs, P, title="Dual-tone chord spectrum",
                                       fig_id="Figure B.3b", f_range=(0, 30))

    return {"separation": separation, "delta_f": delta_f,
            "figs": [fig1, fig2]}
...
def experiment_zero_padding():
    ...
    x, n, t = make_mixed_tones([10.0, 11.0], amplitudes=[1.0, 1.0],
                                duration=short_dur)
    ...
    print(f"Tones at 10 Hz and 11 Hz — separation = 1 Hz vs Δf_min = {delta_f_orig:.2f} Hz")
    ...
    fig1, _ = plot_dual_stack_spectrum(freqs_orig, P_orig,
                                       title="Original (no zero-padding)",
                                       fig_id="Figure B.4a", f_range=f_show)
    fig2, _ = plot_dual_stack_spectrum(freqs_pad, P_pad,
                                       title=f"Zero-padded (×{ZERO_PAD_FACTOR})",
                                       fig_id="Figure B.4b", f_range=f_show)

    return {"delta_f_orig": delta_f_orig, "delta_f_padded": delta_f_padded,
            "figs": [fig1, fig2]}
