...
- **A.6.5** Cross-correlation: comparing two different signals; cross-spectral density (generalized Wiener-Khinchin); Pearson correlation coefficient; application to EEG artifact detection (Volume C)
...
## B.6 — Lab 6: Autocorrelation and Cross-Correlation  *(↔ A.6)*

- **Introduction:** autocorrelation as periodicity detector (A.6.1); Wiener-Khinchin theorem (A.6.2); phase-blindness (A.6.3); cross-correlation for shared structure detection (A.6.5)
- **Setup:** tone buried in noise (same signal as Lab 2: Equation (B.4)); autocorrelation from definition; cross-correlation between two signals with shared/independent components
- **Parameters:** fs, N, f0, A, σ (noise), SNR, seed; cross-correlation: two seeds (42, 99) for independent noise
- **Results:**
  - Experiment A: autocorrelation of tone-in-noise — lag-0 spike vs. periodic peaks
  - Experiment B: Wiener-Khinchin verified numerically (DFT of autocorrelation vs. power spectrum)
  - Experiment C: phase-blindness — two tones with φ = 0 and φ = π produce identical autocorrelations
  - Experiment D: cross-correlation — shared 10 Hz tone detected (ρ = 0.107, periodic peaks); no shared component gives ρ ≈ 0
- **Verification:** r[0] equals total energy (A.54); DFT of r[l] matches power spectrum (A.56); phase-blind (A.6.3); cross-correlation detects shared structure (A.6.5, Eq. A.57-A.59)
- **Conclusion:** autocorrelation exposes periodicity, discards phase; cross-correlation detects shared components between two signals → used in C.4 for artifact identification
