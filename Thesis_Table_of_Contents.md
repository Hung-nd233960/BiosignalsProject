...
- The structure: **Part A** (theory), **Part B** (labs that derive and verify the theory), **Part C** (application to a real EEG signal).
- Each Part A section opens with a one-line **"gap callout"**: what Signals & Systems gives you, and what this section adds.
- Each lab in Part B is cross-referenced to the theory section it tests (↔ A.x).
- Appendix A (the six-signal taxonomy) is the reference grid that every lab and every Part C section points back to.
...
# Part B — Application and Derivation (Labs)
...
# Part C — Application to a Real EEG Signal

> **Approach: adaptive-directed.** The instructor's brief is "show what you find," so the *question* is supplied by the analysis itself. Part C starts by characterizing the data, decides which of the six archetypes the EEG resembles, then deploys the right tool for what was found. The unifying thesis: **EEG can be read as a superposition of the six signal archetypes, and the time-frequency theory of Parts A–B predicts which tool reveals each feature.** Medical associations are *cited and descriptive*, never asserted as diagnoses.
...
- Statistical detection from Part A.4 used to separate signal from artifact
...
A table of the **six signal archetypes** × their behaviour under each transform. Referenced from every lab and every Part C section.
...
*Each cell expanded in the appendix with the formula and the signature; the "why" cross-referenced to Part A.*
...
1. **A↔B pairing is explicit** — every theory section has a matching lab, cross-referenced. The spine A.1→A.2→A.3→A.5→A.6→A.7→A.8 is the irreducible core; A.4/Lab 2 (statistics) is the most detachable if time is short.
2. **Heisenberg (A.5.2) is the conceptual hinge** — it closes the STFT half and motivates the entire WVD half.
3. **Cohen's class (A.8.5) is the unifying frame** — stated once, it ties spectrogram/WVD/PWVD/SPWVD into one family.
4. **Part C is adaptive-directed** — the centerpiece (C.2 vs C.3 vs C.4 vs C.5) is decided by the C.1 triage of the actual EEG, not assumed in advance.
5. **Already built in "Part 3":** A.3, A.5, A.7, A.8 and Labs 3, 4, 6, 7 are largely done — roughly the windowing-through-SPWVD half of the thesis.

---

## Appendix C — EEG Background (domain knowledge for Part C)

> *Purpose: give the reader (and the author) the minimum, cited EEG background needed to interpret Part C honestly. This appendix is descriptive domain knowledge, kept separate from the DSP contribution of Parts A–B. All clinical statements are referenced; nothing here is an original medical claim.*
...
- C.1.3 Sampling, filtering, and referencing as they affect the recorded signal (ties to Part A.1)
...
- C.2.3 Band power as the quantity of interest; why this connects to the DFT (Part A.2, A.4)
...
- C.5.3 What to expect, and which DSP tool each expected feature calls for (the plan that Part C executes)
