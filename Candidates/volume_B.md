...
The DFT and global autocorrelation analyze signal characteristics by averaging over the entire time record, discarding all temporal positioning of frequency components (Lab 6). The STFT spectrogram recovers time-frequency localization by windowing the signal (Lab 4, Lab 5), but its resolution is strictly bounded by the Heisenberg uncertainty principle.

The Wigner-Ville Distribution (WVD) addresses this limitation by taking the Fourier transform of the **instantaneous autocorrelation function (IAF)** (Section A.7.1). For single-component signals, such as a linear chirp, the WVD achieves perfect energy concentration along the signal's instantaneous frequency trajectory, completely bypassing the Heisenberg limit (Section A.7.2). However, for multi-component signals, the WVD's quadratic nature introduces highly energetic, oscillating **cross-term artifacts** centered at the midpoints between genuine components (Section A.7.3, A.7.4). Additionally, computing the WVD on a real-valued signal produces a **DC self-ghost** due to self-interference between positive and negative frequencies, necessitating conversion to the complex-valued **analytic signal** (Section A.7.5).

This lab verifies:
- The WVD achieves razor-sharp time-frequency concentration for a single chirp, far exceeding the STFT spectrogram.
- The WVD of a multi-component signal generates strong, oscillatory cross-term ghosts located at the midpoints of the genuine trajectories.
- Computing the WVD on a real-valued signal produces a heavy DC ghost, which is successfully removed by converting the signal to its analytic form via the Hilbert transform.

### Setup

We construct three test signals to isolate and verify these characteristics:

1. **Experiment A (Single Chirp):** A single linear chirp sweeping from $f_0 = 10$ Hz to $f_{\text{end}} = 80$ Hz over $2.0$ s:
   $$
   x_A[n] = \cos\!\left(\frac{2\pi}{f_s}\left(f_0 n + \frac{\mu}{2} \frac{n^2}{f_s}\right)\right) \tag{B.32}
   $$
   with chirp rate $\mu = (80 - 10)/2.0 = 35$ Hz/s.
2. **Experiment B (Chirp + Tone):** A multi-component signal containing a linear chirp ($f_0 = 10$ Hz to $f_{\text{end}} = 90$ Hz, $\mu = 40$ Hz/s) and a stationary tone at $f_{\text{tone}} = 40$ Hz:
   $$
   x_B[n] = \cos\!\left(\frac{2\pi}{f_s}\left(10 n + 20 \frac{n^2}{f_s}\right)\right) + \cos\!\left(\frac{2\pi \cdot 40}{f_s} n\right) \tag{B.33}
   $$
3. **Experiment C (Real vs. Analytic):** A stationary tone at $f_0 = 30$ Hz:
   $$
   x_C[n] = \cos\!\left(\frac{2\pi \cdot 30}{f_s} n\right) \tag{B.34}
   $$
   This signal is analyzed in its raw real form $x_C[n]$ and in its complex analytic form $z_C[n] = x_C[n] + j\mathcal{H}\{x_C[n]\}$.

All WVD calculations are performed using the implemented `wigner_ville` function in [wvd.py](file:///C:/Users/P1/Downloads/BiosignalsProject-main/BiosignalsProject-main/src/common/wvd.py#L11-L64) which enforces analytic conversion and interpolation-by-2 to prevent frequency-aliasing.

### Parameters

**Table B.10 - Lab 7 parameters**

| Parameter | Value | Description |
| --- | --- | --- |
| $f_s$ | 250 Hz | Sampling frequency |
| Duration | 2.0 s | Signal duration |
| $N$ | 500 | Number of signal samples |
| $N_f$ | 512 | FFT length for spectral computation |
| $\text{Exp A: } f_0, f_{\text{end}}$ | 10.0 Hz, 80.0 Hz | Chirp frequency range |
| $\text{Exp B: } f_0, f_{\text{end}}$ | 10.0 Hz, 90.0 Hz | Chirp frequency range |
| $\text{Exp B: } f_{\text{tone}}$ | 40.0 Hz | Stationary tone frequency |
| $\text{Exp C: } f_0$ | 30.0 Hz | Test tone frequency |

### Results

**Experiment A - Single Chirp Resolution.**

Figure B.41 shows the linear chirp in the time domain, displaying a smooth, continuous increase in oscillation frequency over the 2.0-second interval.

![Figure B.41 - Linear chirp time-domain signal](../results/graphs/lab7/figure_B_41.png)

Figure B.42 displays the time-frequency representation computed via the STFT (top panel) and the WVD (bottom panel). The STFT spectrogram (Hanning window, length 64 samples = 0.256 s) shows a thick, blurred diagonal trajectory. The time-frequency resolution is visibly smeared due to the windowing compromise. In contrast, the WVD (bottom panel) maps the chirp as a **razor-sharp diagonal line**. The energy is concentrated entirely in the bins corresponding to the instantaneous frequency $f_{\text{inst}}(t) = 10 + 35t$ Hz, verifying the theoretical prediction (Equation (A.56)).

![Figure B.42 - Single chirp: blurred STFT vs. razor-sharp WVD](../results/graphs/lab7/figure_B_42.png)

**Experiment B - Multi-component Cross-Terms.**

Figure B.43 compares the STFT and WVD for the combined chirp-plus-tone signal. The STFT (top panel) displays the two components as a blurred diagonal line (chirp) and a blurred horizontal line (40 Hz tone) crossing at $t = 0.75$ s. Crucially, the STFT represents a simple, clean linear superposition of the two components.

The WVD (bottom panel) resolves both trajectories with high sharpness, but is **heavily corrupted by a third, highly energetic, oscillating trajectory**. This is the cross-term ghost:
- **Location:** It is located at the frequency midpoint $f_c(t) = \frac{10 + 40t + 40}{2} = 25 + 20t$ Hz, tracking exactly halfway between the chirp and the tone. At $t = 0$ s, it starts at 25 Hz; it crosses the 40 Hz tone at $t = 0.75$ s (where the chirp is also at 40 Hz); and it ends at 45 Hz at $t = 2.0$ s.
- **Oscillations:** It oscillates in time along the $t$-axis. The frequency of these oscillations increases as the distance between the two components grows, matching the difference frequency $f_{\text{diff}}(t) = |40t - 30|$ Hz, as predicted by Equation (A.61).

![Figure B.43 - Chirp + Tone: clean STFT superposition vs. WVD cross-term corruption](../results/graphs/lab7/figure_B_43.png)

This experiment confirms that the WVD cannot be applied directly to multi-component signals without generating severe ghost artifacts that can easily be mistaken for genuine signal components.

**Experiment C - Real vs. Analytic WVD.**

Figure B.44 compares the WVD of a real-valued 30 Hz tone (top panel) against the WVD of its complex-valued analytic counterpart (bottom panel).
- The WVD of the real signal (top panel) is corrupted by a **dominant, highly energetic, oscillating band at 0 Hz (DC)**. This is the DC self-ghost predicted in Section A.7.5, resulting from cross-term interference between the positive (+30 Hz) and negative (-30 Hz) frequency components of the real cosine. The artifact oscillates in time at $2f_0 = 60$ Hz and completely masks the low-frequency region of the time-frequency plane.
- The WVD of the analytic signal (bottom panel), computed by applying the Hilbert transform to remove negative frequencies, shows a **completely clean horizontal line at 30 Hz** with zero energy at DC.

![Figure B.44 - WVD of 30 Hz tone: real signal (heavy DC ghost) vs. analytic signal (clean)](../results/graphs/lab7/figure_B_44.png)

This confirms that converting the real signal to its analytic form is mandatory to remove negative-frequency self-interference.

### Verification

The experimental measurements are verified against the mathematical models developed in Volume A:

| Feature / Prediction | Theoretical Model | Measured Result | Confirmed? |
| --- | --- | --- | --- |
| Chirp Resolution | Razor-sharp trajectory (Eq. A.56) | Sharp diagonal, no window blur | Yes |
| Cross-term Midpoint | Midpoint trajectory: $25 + 20t$ Hz | Trajectory matches $25 + 20t$ Hz | Yes |
| Cross-term Oscillation | Oscillations at $|40t - 30|$ Hz | Oscillations increase with distance | Yes |
| Real Signal DC Ghost | Time-oscillating ghost at 0 Hz (A.7.5) | Heavy 0 Hz band oscillating at 60 Hz | Yes |
| Analytic Signal Remedy | Complete ghost removal (Eq. A.63) | Clean 30 Hz line, 0 Hz band eliminated | Yes |

### Conclusion

Lab 7 confirms both the strength and the central weakness of the raw Wigner-Ville Distribution. It achieves perfect, sub-Heisenberg concentration for single-component non-stationary signals, mapping a linear chirp to a line. However, the moment a second component is introduced, the bilinear product creates highly energetic cross-term ghosts that corrupt the representation. Additionally, the real-valued formulation produces a severe DC ghost.

Converting the signal to its analytic form removes positive-to-negative self-ghosts, but does not affect cross-terms between separate physical components (such as a chirp and a tone). To suppress these multi-component ghosts while preserving the high resolution of the WVD, we must introduce independent smoothing operations. This is the focus of Lab 8.
...
Lab 7 demonstrated that while the WVD provides unmatched resolution for a single component, it is severely limited by cross-term artifacts in multi-component scenarios.

The **Smoothed Pseudo Wigner-Ville Distribution (SPWVD)** resolves this problem by low-pass filtering the time-frequency plane (Section A.8.3). It decouples the filtering axes using two independent smoothing windows:
- A **lag window** $h[m]$ of length $2M+1$ that windows the autocorrelation in the lag domain. This is mathematically equivalent to convolving the WVD along the frequency axis ($f$-axis), suppressing cross-terms that oscillate in frequency (Section A.8.1).
- A **time window** $g[p]$ of length $2P+1$ that averages the autocorrelation in time. This is equivalent to convolving the WVD along the time axis ($t$-axis), suppressing cross-terms that oscillate in time (Section A.8.3).

This lab verifies:
- The progression from WVD to PWVD to SPWVD step-by-step suppresses frequency-oscillating and time-oscillating ghosts.
- The smoothing windows $h$ and $g$ act as independent knobs, allowing time and frequency resolution to be traded off separately along each axis (Section A.8.4).
- The *minimum smoothing rule* (Equation (A.66)) governs the suppression of time-oscillating cross-terms.

### Setup

We use the multi-component signal from Lab 7 Experiment B: a linear chirp (10 to 90 Hz) plus a stationary tone at 40 Hz (Equation (B.33)), sampled at $f_s = 250$ Hz over a duration of 2.0 s ($N = 500$ samples).

We conduct two experiments:
1. **Experiment A (Progression):** Compute and compare three distributions:
   - **Raw WVD:** No smoothing (equivalent to $g[p] = \delta[p]$ and $h[m] = 1$).
   - **PWVD:** Lag-smoothed only using a Hann window $h$ of length 51 samples, and no time window ($g$ length 1).
   - **SPWVD:** Both lag-smoothed ($h$ Hann, length 51) and time-smoothed ($g$ Hann, length 15).
2. **Experiment B (Smoothing Sweep):** Test two extreme cases of SPWVD window tuning:
   - **Case 1 (Strong Frequency Smoothing, Weak Time Smoothing):** $h$ length 101, $g$ length 5.
   - **Case 2 (Weak Frequency Smoothing, Strong Time Smoothing):** $h$ length 25, $g$ length 31.

All calculations are performed using the `smoothed_pseudo_wigner_ville` function in [wvd.py](file:///C:/Users/P1/Downloads/BiosignalsProject-main/BiosignalsProject-main/src/common/wvd.py#L67-L174).

### Parameters

**Table B.11 - Lab 8 parameters**

| Parameter | Value / Window | Description |
| --- | --- | --- |
| $f_s$ | 250 Hz | Sampling frequency |
| Duration | 2.0 s | Signal duration |
| $N$ | 500 | Signal samples |
| $N_f$ | 512 | FFT length for spectral computation |
| $\text{Exp A - WVD}$ | $h = 1, g = 1$ | No smoothing |
| $\text{Exp A - PWVD}$ | $h$: Hann 51 ($T_h \approx 0.20$ s), $g$: None | Lag-smoothed only |
| $\text{Exp A - SPWVD}$ | $h$: Hann 51, $g$: Hann 15 ($T_g \approx 0.06$ s) | Time and lag smoothed |
| $\text{Exp B - Case 1}$ | $h$: Hann 101 ($0.40$ s), $g$: Hann 5 ($0.02$ s) | Strong frequency, weak time smoothing |
| $\text{Exp B - Case 2}$ | $h$: Hann 25 ($0.10$ s), $g$: Hann 31 ($0.12$ s) | Weak frequency, strong time smoothing |

### Results

**Experiment A - WVD $\to$ PWVD $\to$ SPWVD Progression.**

Figure B.51 shows the progression from raw WVD (top panel) to PWVD (middle panel) to SPWVD (bottom panel) for the chirp-plus-tone signal.
- **1. Raw WVD (top panel):** Sharp trajectories but heavily corrupted by the oscillating midpoint cross-term ghost, which spans the entire time-frequency plane.
- **2. PWVD (middle panel):** The lag window $h=51$ performs frequency smoothing. The frequency-oscillating ghosts are successfully smoothed out. However, the time-oscillating cross-terms survive completely intact, appearing as a prominent, highly active oscillating band at the midpoint frequency.
- **3. SPWVD (bottom panel):** The addition of the time-smoothing window $g=15$ filters along the time axis. The time-oscillating cross-terms are **completely suppressed**, leaving a clean representation showing only the two true components: the linear chirp and the stationary 40 Hz tone. The resolution of both components remains significantly sharper than that of the STFT spectrogram (compare with Figure B.43, top panel).

![Figure B.51 - WVD to PWVD to SPWVD progression: step-by-step ghost suppression](../results/graphs/lab8/figure_B_51.png)

This progression confirms that joint time-frequency smoothing is required to suppress multi-component cross-terms, and that the SPWVD successfully achieves this while preserving high trajectory resolution.

**Experiment B - Window Length Sweep.**

Figure B.52 displays the results of the window sweep, showing the independent nature of the time and frequency smoothing axes.
- **Case 1 (h=101, g=5; top panel):** Uses a very long lag window $h$ and a very short time window $g$. Because $h$ is long, frequency smoothing is minimal, resulting in **very high frequency resolution** (the 40 Hz tone is a thin line). However, because the time window $g$ is short ($T_g = 5/250 = 0.02$ s), it fails the minimum smoothing rule (Equation (A.66)) for time oscillations. The difference frequency between the chirp and the tone ranges from 10 to 50 Hz, requiring a time window of at least $T_g \ge 1/10 = 0.10$ s to suppress the cross-term at all times. Since $0.02 \text{ s} < 0.10$ s, strong time-oscillating cross-terms survive and corrupt the plot.
- **Case 2 (h=25, g=31; bottom panel):** Uses a short lag window $h$ and a long time window $g$. The long time window ($T_g = 31/250 = 0.124$ s) satisfies the minimum smoothing rule ($T_g > 0.10$ s), so the **time-oscillating cross-terms are completely suppressed**. However, the short lag window $h=25$ introduces heavy frequency smoothing, which **blurs the frequency resolution** (the 40 Hz tone is smeared into a thick band, and the chirp trajectory is thickened).

![Figure B.52 - SPWVD window sweep: Case 1 (h=101, g=5) vs. Case 2 (h=25, g=31)](../results/graphs/lab8/figure_B_52.png)

This sweep visually demonstrates that the SPWVD's smoothing knobs are independent. Unlike the STFT, where resolution is symmetric and bound, the SPWVD allows us to selectively trade time sharpness for frequency sharpness by adjusting the length of $g$ and $h$ independently.

### Verification

The experimental findings verify the smoothing theory of Volume A:

| Feature / Prediction | Theoretical Model | Measured Result | Confirmed? |
| --- | --- | --- | --- |
| PWVD Limitation | Time-oscillating ghosts survive (A.8.2) | Midpoint ghost remains (Fig. B.51, middle) | Yes |
| SPWVD Remedy | Dual-window smoothing suppresses all ghosts | Clean, ghost-free plot (Fig. B.51, bottom) | Yes |
| Min. Smoothing Rule | $T_g \ge 1/\Delta f$ needed to kill ghosts (Eq. A.66) | Case 1 ($T_g = 0.02$ s) shows residual ghosts | Yes |
| Axis Decoupling | $h$ controls $\Delta f$, $g$ controls $\Delta t$ independently | Independent blurring along axes (Fig. B.52) | Yes |

### Conclusion

Lab 8 confirms that the Smoothed Pseudo Wigner-Ville Distribution is a highly flexible tool for time-frequency analysis. By decoupling the smoothing operations in time and frequency, it suppresses the cross-terms that ruin the raw WVD, while allowing the engineer to tune the resolution along each axis independently.

This flexibility is highly valuable for real-world signals like EEG. For example, to resolve a brief, non-stationary sleep spindle (requiring high time resolution) coexisting with a stationary alpha rhythm (requiring high frequency resolution), the SPWVD windows can be tuned to optimize each axis separately. This is the payoff of the WVD family, which we will test on real EEG data in Volume C.
