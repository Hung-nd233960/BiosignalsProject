...
**The $N$ vs $N-1$ convention.** Equation (A.16) divides by $N$ in the cosine argument. Many textbooks write $N-1$ instead:
...
**Table A.1a - Library defaults for window conventions**
...
Table A.1 lists the four standard members, their coefficients, and their spectral properties. The edge values in Table A.1 refer to the periodic convention.
...
The global autocorrelation $r[l]$ (Section A.6) compares a signal with its shifted copy over the entire time record, collapsing the time index and yielding a single vector indexed only by lag. To construct a time-varying spectrum, we require a localized measure of self-similarity. This is achieved by defining the **instantaneous autocorrelation function (IAF)**.

In the continuous-time domain, the IAF of a signal $x(t)$ is defined as:

$$
R_x(t, \tau) = x(t + \tau/2) \, x^*(t - \tau/2) \tag{A.48}
$$

where $t$ is the physical time and $\tau$ is the time lag. The product is symmetric about the current time $t$, evaluating the signal's self-similarity at equal distances into the future ($+\tau/2$) and the past ($-\tau/2$). The continuous-time **Wigner-Ville Distribution (WVD)** is defined as the Fourier transform of this instantaneous autocorrelation with respect to the lag $\tau$:

$$
W_x(t, f) = \int_{-\infty}^{\infty} x(t + \tau/2) \, x^*(t - \tau/2) \, e^{-j 2\pi f \tau} \, d\tau \tag{A.49}
$$

To transition to the discrete realm, we must evaluate the signal at discrete indices. A naive discretization of Equation (A.48) would evaluate $x[n + m/2] x^*[n - m/2]$, which requires half-integer sample values when the lag index $m$ is odd. To maintain calculations strictly on the integer sample grid, the **discrete-time instantaneous autocorrelation** is defined with a double-lag step:

$$
R_x[n, m] = x[n + m] \, x^*[n - m] \tag{A.50}
$$

where $n$ is the discrete time sample and $m$ is the lag. The **Discrete-Time Wigner-Ville Distribution (DT-WVD)** is the discrete-time Fourier transform (DTFT) of this sequence with respect to $m$:

$$
W_x[n, \theta] = 2 \sum_{m=-\infty}^{\infty} x[n + m] \, x^*[n - m] \, e^{-j 2 \theta m} \tag{A.51}
$$

where $\theta = 2\pi f / f_s$ is the normalized angular frequency. The scaling factor of 2 is a standard convention to align the energy scale with the continuous distribution. For a finite signal on a discrete grid, using a maximum lag search range $M$ and computing the distribution over $N_f$ discrete frequency bins, we arrive at the practical computation formula:

$$
W_x[n, k] = 2 \sum_{m=-M}^{M} x[n + m] \, x^*[n - m] \, e^{-j \frac{4\pi k m}{N_f}} \tag{A.52}
$$

**The Frequency Aliasing Problem.** The double-lag step $x[n+m]x^*[n-m]$ in Equation (B.50) has a critical mathematical consequence. By shifting the signal in steps of $+m$ and $-m$, the spacing between the two samples is $2m$. The effective sampling interval of this sequence is therefore $2\Delta t$, which is twice that of the original signal. By the Nyquist-Shannon sampling theorem, doubling the sampling interval halves the Nyquist frequency. The spectral range of the DT-WVD is compressed from $[-f_s/2, f_s/2]$ to $[-f_s/4, f_s/4]$. Any frequency component higher than $f_s/4$ will wrap around (alias) into the lower half of the spectrum.

To prevent this aliasing, the input signal must be **interpolated by a factor of 2** prior to computing the autocorrelation. In the interpolated signal (sampled at $2f_s$), the lag step $m$ corresponds to a physical spacing of $\Delta t$ (rather than $2\Delta t$), which restores the Nyquist limit to $f_s/2$. The implementation in [wvd.py](file:///C:/Users/P1/Downloads/BiosignalsProject-main/BiosignalsProject-main/src/common/wvd.py#L35-L38) enforces this interpolation using Fourier-domain zero-padding (`scipy.signal.resample`), ensuring exact physical frequency mapping up to $f_s/2$.

### A.7.2 Sharpness of the Single Chirp

The principal advantage of the WVD over the STFT is its ability to bypass the Heisenberg uncertainty principle (Equation (A.40)) for single-component non-stationary signals. This is best illustrated by evaluating the WVD of a continuous-time linear chirp:

$$
x(t) = A e^{j 2\pi \left(f_0 t + \frac{1}{2}\mu t^2\right)} \tag{A.53}
$$

where $f_0$ is the initial frequency and $\mu$ is the chirp rate. We compute its continuous instantaneous autocorrelation:

$$
R_x(t, \tau) = x(t + \tau/2) \, x^*(t - \tau/2) = A^2 e^{j 2\pi \left(f_0 (t + \tau/2) + \frac{1}{2}\mu (t + \tau/2)^2\right)} e^{-j 2\pi \left(f_0 (t - \tau/2) + \frac{1}{2}\mu (t - \tau/2)^2\right)}
$$

Expanding the phase terms in the exponent:

$$
\phi(t + \tau/2) - \phi(t - \tau/2) = 2\pi f_0 \left[(t + \tau/2) - (t - \tau/2)\right] + \pi\mu \left[(t + \tau/2)^2 - (t - \tau/2)^2\right]
$$
$$
= 2\pi f_0 \tau + \pi\mu \left[\left(t^2 + t\tau + \frac{\tau^2}{4}\right) - \left(t^2 - t\tau + \frac{\tau^2}{4}\right)\right]
$$
$$
= 2\pi (f_0 + \mu t) \tau \tag{A.54}
$$

Substituting this back into the continuous WVD definition (Equation (A.49)):

$$
W_x(t, f) = \int_{-\infty}^{\infty} A^2 e^{j 2\pi (f_0 + \mu t) \tau} \, e^{-j 2\pi f \tau} \, d\tau = A^2 \int_{-\infty}^{\infty} e^{-j 2\pi (f - (f_0 + \mu t)) \tau} \, d\tau \tag{A.55}
$$

Since the integral of a complex exponential over its full range is a Dirac delta function:

$$
W_x(t, f) = A^2 \, \delta\left(f - (f_0 + \mu t)\right) \tag{A.56}
$$

Equation (A.56) is a remarkable result. The time-frequency distribution of a linear chirp is a **perfectly concentrated line** (a delta function) along the exact trajectory of its instantaneous frequency $f_{\text{inst}}(t) = f_0 + \mu t$. There is no smearing in time and no smearing in frequency. The WVD achieves infinite resolution on this archetype, completely overcoming the trade-off that limits the STFT (where short windows blur the sweep vertically and long windows blur it horizontally).

### A.7.3 The Quadratic Nature and Cross-Terms

The infinite resolution of the WVD is made possible by its **quadratic (bilinear) structure**: the signal is multiplied by itself. This multiplication, however, introduces a severe complication when a signal contains more than one component. The WVD is non-linear; the superposition principle does not apply.

Let a multi-component signal be $x(t) = x_1(t) + x_2(t)$. Its instantaneous autocorrelation is:

$$
R_x(t, \tau) = \left[x_1(t+\tau/2) + x_2(t+\tau/2)\right] \left[x_1^*(t-\tau/2) + x_2^*(t-\tau/2)\right]
$$
$$
= x_1(t+\tau/2)x_1^*(t-\tau/2) + x_2(t+\tau/2)x_2^*(t-\tau/2) + x_1(t+\tau/2)x_2^*(t-\tau/2) + x_2(t+\tau/2)x_1^*(t-\tau/2)
$$
$$
= R_{x_1}(t,\tau) + R_{x_2}(t,\tau) + 2 \text{Re}\left\{x_1(t+\tau/2)x_2^*(t-\tau/2)\right\} \tag{A.57}
$$

Taking the Fourier transform of Equation (A.57) yields:

$$
W_x(t, f) = W_{x_1}(t, f) + W_{x_2}(t, f) + 2 \text{Re}\left\{W_{x_1 x_2}(t, f)\right\} \tag{A.58}
$$

where the third term is the **cross-Wigner-Ville distribution** (or cross-term):

$$
W_{x_1 x_2}(t, f) = \int_{-\infty}^{\infty} x_1(t + \tau/2) \, x_2^*(t - \tau/2) \, e^{-j 2\pi f \tau} \, d\tau \tag{A.59}
$$

To understand the structure of these cross-terms, consider two pure, complex-valued tones: $x_1(t) = A_1 e^{j (2\pi f_1 t + \phi_1)}$ and $x_2(t) = A_2 e^{j (2\pi f_2 t + \phi_2)}$. We compute their cross-WVD:

$$
W_{x_1 x_2}(t, f) = \int_{-\infty}^{\infty} A_1 e^{j \left(2\pi f_1 (t + \tau/2) + \phi_1\right)} \cdot A_2 e^{-j \left(2\pi f_2 (t - \tau/2) + \phi_2\right)} e^{-j 2\pi f \tau} \, d\tau
$$
$$
= A_1 A_2 e^{j \left(2\pi (f_1 - f_2) t + (\phi_1 - \phi_2)\right)} \int_{-\infty}^{\infty} e^{-j 2\pi \left(f - \frac{f_1 + f_2}{2}\right) \tau} \, d\tau
$$
$$
= A_1 A_2 e^{j \left(2\pi (f_1 - f_2) t + (\phi_1 - \phi_2)\right)} \, \delta\left(f - \frac{f_1 + f_2}{2}\right) \tag{A.60}
$$

Adding the symmetric conjugate term $W_{x_2 x_1}(t, f)$ to get the real-valued cross-term:

$$
2 \text{Re}\left\{W_{x_1 x_2}(t, f)\right\} = 2 A_1 A_2 \, \delta\left(f - \frac{f_1 + f_2}{2}\right) \cos\left(2\pi (f_1 - f_2) t + (\phi_1 - \phi_2)\right) \tag{A.61}
$$

Equation (A.61) defines the three fundamental properties of WVD cross-terms:

1. **Midpoint Location:** The cross-term is located at $f_c = (f_1 + f_2)/2$, exactly halfway between the two true components in frequency.
2. **Temporal Oscillation:** The cross-term oscillates in the time direction ($t$-axis) with an oscillation frequency equal to the difference frequency $f_{\text{diff}} = |f_1 - f_2|$.
3. **Amplitude Scaling:** The peak amplitude of the cross-term is $2 A_1 A_2$, which is twice the product of the component amplitudes. If $A_1 = A_2 = 1$, the cross-term has a peak value of 2, twice as large as the genuine auto-terms.

These cross-terms are not noise; they are structured, highly energetic mathematical artifacts. For a signal containing $K$ components, there are $K$ auto-terms and $K(K-1)/2$ cross-terms. In a real-world multi-component signal like EEG, these cross-terms quickly multiply, forming a dense "cross-term soup" that completely obscures the true signal features.

### A.7.4 Duality of Cross-Terms (Tones vs. Impulses)

Cross-terms exhibit a strict mathematical duality between the time and frequency domains. We compare the two extreme archetypes of Table AA.1: two stationary tones (narrowband in frequency, delocalized in time) and two impulses (narrowband in time, broadband in frequency).

**Case A: Two Tones.** As derived in Equation (A.61), two tones separated in frequency by $\Delta f = |f_1 - f_2|$ create a cross-term that is **localized in frequency** (at the midpoint $f_c$) and **oscillates in time** with frequency $\Delta f$.

**Case B: Two Impulses.** Let $x(t) = \delta(t - t_1) + \delta(t - t_2)$ be two impulses separated in time by $\Delta t = |t_1 - t_2|$. Evaluating their cross-WVD yields:

$$
2 \text{Re}\left\{W_{x_1 x_2}(t, f)\right\} = 2 \, \delta\left(t - \frac{t_1 + t_2}{2}\right) \cos\left(2\pi (t_1 - t_2) f\right) \tag{A.62}
$$

Here, the cross-term is **localized in time** at the midpoint $t_c = (t_1 + t_2)/2$ and **oscillates in frequency** with a frequency of oscillation equal to the time spacing $\Delta t$.

**Table A.10 - Duality of WVD cross-terms**

| Separation Type | Midpoint Location | Oscillation Direction | Oscillation Frequency |
| --- | --- | --- | --- |
| Two Tones ($\Delta f$) | Midpoint Frequency $f_c = (f_1 + f_2)/2$ | Time ($t$-axis) | $f_{\text{osc}} = \Delta f$ (Hz) |
| Two Impulses ($\Delta t$) | Midpoint Time $t_c = (t_1 + t_2)/2$ | Frequency ($f$-axis) | $t_{\text{osc}} = \Delta t$ (s) |

For a general time-frequency separation where two components are separated by both $\Delta t$ and $\Delta f$, the cross-term is centered at their joint time-frequency midpoint $(t_c, f_c)$, and its oscillations occur in a direction perpendicular to the line connecting the two auto-terms.

### A.7.5 Analytic Signal (Hilbert Transform) and DC Self-Ghost

A particularly damaging manifestation of the cross-term problem occurs when computing the WVD of a real-valued signal. By Euler's formula, a real-valued tone $x(t) = A \cos(2\pi f_0 t)$ consists of two complex-exponential components: one at positive frequency $+f_0$ and one at negative frequency $-f_0$.

Because the WVD is quadratic, it treats these positive and negative frequencies as two separate components, generating a cross-term between them. Applying Equation (A.61) with $f_1 = f_0$ and $f_2 = -f_0$ yields:

- **Midpoint Location:** $f_c = (f_0 - f_0)/2 = 0$ Hz (DC).
- **Oscillation Frequency:** $f_{\text{diff}} = |f_0 - (-f_0)| = 2f_0$ Hz.

This is the **DC self-ghost**. Any real-valued tone at frequency $f_0$ produces an oscillating artifact centered at $0$ Hz with frequency $2f_0$. In biomedical applications like EEG, where low-frequency activity (Delta band, 1–4 Hz) is clinically significant, the DC self-ghost of alpha or beta rhythms would completely corrupt the low-frequency plane.

**The Solution: The Analytic Signal.** To eliminate the DC self-ghost, the real-valued signal $x(t)$ must be converted to its complex-valued **analytic signal** $z(t)$ before computing the WVD:

$$
z(t) = x(t) + j \mathcal{H}\left\{x(t)\right\} \tag{A.63}
$$

where $\mathcal{H}\{\cdot\}$ is the Hilbert transform. The Hilbert transform shifts the phase of all positive frequency components by $-90^\circ$ and all negative frequency components by $+90^\circ$. Adding $j\mathcal{H}\{x(t)\}$ to $x(t)$ cancels all negative frequency components, leaving a spectrum that exists only on the positive frequency half-plane $[0, f_s/2]$. Because there are no negative frequencies, the cross-term between $+f_0$ and $-f_0$ cannot form, and the DC self-ghost is eliminated. This step is mandatory for any practical WVD implementation and is executed at the very beginning of the algorithms in [wvd.py](file:///C:/Users/P1/Downloads/BiosignalsProject-main/BiosignalsProject-main/src/common/wvd.py#L29-L30).
...
To make the Wigner-Ville Distribution a useful tool for multi-component signals, we must suppress the cross-terms. Since cross-terms are highly oscillatory, they can be suppressed by low-pass filtering (smoothing) the time-frequency plane.

The **Pseudo Wigner-Ville Distribution (PWVD)** introduces smoothing by windowing the instantaneous autocorrelation sequence in the lag domain. We define a symmetric lag window $h[m]$ of length $2M+1$, centered at $m=0$. The DT-PWVD is computed as:

$$
PW_x[n, k] = 2 \sum_{m=-M}^M h[m] \, x[n+m] \, x^*[n-m] \, e^{-j \frac{4\pi k m}{N_f}} \tag{A.64}
$$

By the modulation theorem, multiplication in the lag domain corresponds to convolution in the frequency domain. The PWVD is mathematically equivalent to convolving the raw WVD with the Fourier transform of the lag window $H(f)$:

$$
PW_x[n, f] = \int_{-f_s/2}^{f_s/2} W_x[n, \lambda] \, H(f - \lambda) \, d\lambda = W_x[n, f] *_f H(f) \tag{A.65}
$$

This convolution acts as a low-pass filter along the frequency axis ($f$-axis). According to Section A.7.4, cross-terms that oscillate in the frequency direction are generated by components separated in time (such as two consecutive impulses). The frequency smoothing introduced by $h[m]$ smears these oscillations out, effectively killing the frequency-oscillating ghosts.

### A.8.2 Limitation: Time-Oscillating Ghosts

While the PWVD successfully suppresses cross-terms that oscillate along the frequency axis, it has a severe limitation: **it performs no smoothing along the time axis**.

Cross-terms that oscillate along the time axis ($t$-axis) are generated by components separated in frequency (such as two simultaneous tones, per Equation (A.61)). Because the lag window $h[m]$ only filters the lag index $m$ at a fixed time sample $n$, it cannot suppress oscillations that evolve across different values of $n$. As a result, time-oscillating ghosts survive the PWVD completely intact, rendering it ineffective for signals containing simultaneous rhythms (a common occurrence in EEG).

### A.8.3 The Time Window $g$ (Time Smoothing)

To suppress both time-oscillating and frequency-oscillating cross-terms, we must perform smoothing along both axes of the time-frequency plane. This is achieved by the **Smoothed Pseudo Wigner-Ville Distribution (SPWVD)**.

The SPWVD introduces a second window: a symmetric **time-smoothing window** $g[p]$ of length $2P+1$, which averages the instantaneous autocorrelation across adjacent time steps before computing the Fourier transform. The discrete-time SPWVD is defined as:

$$
SPW_x[n, k] = 2 \sum_{m=-M}^M h[m] \left( \sum_{p=-P}^P g[p] \, x[n - p + m] \, x^*[n - p - m] \right) e^{-j \frac{4\pi k m}{N_f}} \tag{A.65}
$$

The double summation divides the smoothing operations:
1. The inner sum convolves the autocorrelation sequence in time with the window $g[p]$, smoothing along the $t$-axis.
2. The outer sum multiplies by the lag window $h[m]$ and computes the DFT, smoothing along the $f$-axis.

This structure provides **two independent smoothing knobs**: the time window $g$ (filtering time oscillations) and the lag window $h$ (filtering frequency oscillations). This dual-filtering suppresses cross-terms regardless of whether their components are separated in time, frequency, or both.

### A.8.4 The Smoothing Tradeoff

Smoothing is not a free lunch; it suppresses artifacts by discarding high-frequency detail. Convolving with $g[p]$ smears the signal in time, degrading the time resolution. Convolving with $h[m]$ smears the signal in frequency, degrading the frequency resolution.

However, the SPWVD offers a fundamental advantage over the STFT. In the STFT, time and frequency resolutions are strictly tied by a single window: shortening the window improves time resolution but degrades frequency resolution. In the SPWVD, the smoothing operations are decoupled:

- The time resolution is determined solely by the width of the time window $g[p]$.
- The frequency resolution is determined solely by the width of the lag window $h[m]$.

We can choose the window lengths $2P+1$ and $2M+1$ independently to optimize the trade-off for a specific signal.

**The Minimum Smoothing Rule.** To suppress a cross-term, the smoothing window must be wide enough to average out at least one full cycle of its oscillation.
- To suppress a time-oscillating ghost from components separated by $\Delta f$ Hz, the duration of the time window $g$ must satisfy:
  $$
  T_g \ge \frac{1}{\Delta f} \quad \text{(s)} \tag{A.66}
  $$
- To suppress a frequency-oscillating ghost from components separated by $\Delta t$ s, the bandwidth of the lag window's spectrum $H(f)$ must satisfy:
  $$
  B_H \ge \frac{1}{\Delta t} \quad \text{(Hz)} \tag{A.67}
  $$

If the window is narrower than this limit, the oscillation is not fully averaged, and residual ghosts will remain. Hann or Gaussian windows are preferred for both $g$ and $h$ because their Fourier transforms decay rapidly and do not have strong side lobes, preventing the introduction of secondary ripples (ringing) in the smoothed output.

### A.8.5 Cohen's Class: A Unifying Framework

The relationship between the WVD, PWVD, SPWVD, and Spectrogram is formalized by **Cohen's class of quadratic time-frequency distributions**. Any time-frequency distribution $C_x(t, f)$ that is invariant to time and frequency shifts can be written as a 2D convolution of the raw WVD with a kernel function $\Pi(t, f)$:

$$
C_x(t, f) = \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} \Pi(t - u, f - v) \, W_x(u, v) \, du \, dv \tag{A.68}
$$

The kernel $\Pi(t, f)$ defines the smoothing characteristics of the distribution, acting as a 2D low-pass filter on the WVD. Different kernels yield different distributions:

1. **The Raw WVD:** The kernel is a 2D delta function: $\Pi(t, f) = \delta(t) \delta(f)$. No smoothing is performed. It achieves maximum resolution but suffers from maximum cross-term corruption.
2. **The Spectrogram:** The kernel is the WVD of the spectrogram window $w_k(t)$ itself: $\Pi(t, f) = W_{w_k}(t, -f)$. Because the window's WVD satisfies the Heisenberg uncertainty limit, the spectrogram represents a heavily smoothed distribution where all cross-terms are suppressed, but at the cost of severe, coupled resolution loss.
3. **The SPWVD:** The kernel is separable: $\Pi(t, f) = g(t) H(f)$, where $g(t)$ is a time-domain window and $H(f)$ is the Fourier transform of a frequency-domain window. This separates the smoothing axes, allowing independent resolution tuning.

Cohen's class reveals that the raw WVD and the Spectrogram are not two unrelated transforms, but rather the two boundary points of a single mathematical family. The SPWVD occupies the space between them, offering a configurable bridge that allows engineers to trade resolution for ghost suppression in a controlled, axis-independent manner.
